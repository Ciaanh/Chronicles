...
Events follow this actual database structure:

```lua
{
    id = number,                    -- Unique numeric identifier
    label = string,                 -- Localized event title (e.g., "LABEL_EVENT_001")
    chapters = {                    -- Event content organized in chapters
        {
            header = string,        -- Chapter title (localized key)
            pages = {              -- Array of content pages
                string,            -- Each page is a localized text key
                string,
                -- ... more pages
            }
        },
    ...
    yearStart = number,            -- Starting year of the event
    yearEnd = number,              -- Ending year of the event (optional, can be same as yearStart)
    eventType = number,            -- Event type identifier (1-10+ for different categories)
    timeline = number,             -- Timeline identifier for organization
    order = number,                -- Display order within the timeline
    characters = {},               -- Array of character IDs (currently empty in most events)
    factions = {},                 -- Array of faction IDs (currently empty in most events)
    author = string                -- Optional author field for event attribution
}
```

### Clean Event Object Structure

The Core system processes raw database events into clean objects with this structure:

```lua
{
    id = number,                    -- Event ID
    type = "event",                 -- Object type identifier
    label = string,                 -- Localized label key
    years = {                       -- Year range information
        yearStart = number,
        yearEnd = number
    },
    data = {                        -- Core event data
        eventType = number,
        timeline = number,
        order = number,
        chapters = {},              -- Processed chapter data
        characters = {},            -- Associated character IDs
...
Characters follow this database structure:

```lua
{
    id = number,                    -- Unique numeric identifier
    name = string,                  -- Localized character name (e.g., "LABEL_CHARACTER_001")
    chapters = {                    -- Character lore organized in chapters
        {
            header = string,        -- Chapter title (localized key)
            pages = {              -- Array of content pages
                string,            -- Each page is a localized text key
    ...
    timeline = number,             -- Timeline identifier for organization
    factions = {}                  -- Array of faction IDs this character is associated with
}
```

### Faction Data Structure

Factions follow this database structure:

```lua
{
    id = number,                    -- Unique numeric identifier
    name = string,                  -- Localized faction name (e.g., "LABEL_FACTION_001")
    chapters = {                    -- Faction lore organized in chapters
        {
            header = string,        -- Chapter title (localized key)
            pages = {              -- Array of content pages
                string,            -- Each page is a localized text key
    ...
    timeline = number,             -- Timeline identifier for organization
    characters = {}                -- Array of character IDs associated with this faction
}
```
...
-   Empty arrays for `characters` and `factions` indicate future expansion capability
...
2. Test all functionality after major WoW updates
3. Update dependencies as needed
4. Review and update event data for accuracy
