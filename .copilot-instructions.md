...
-   **Database**: Chronicles.DB (maintained in DB/ folder structure)
...
│       ├── ContentUtils.lua         # Content transformation utilities (UI/Book/ContentUtils.lua)
...
│       ├── BookContainerTemplate.*     # Book-style display
...
-   `CreateUnifiedContent(entity)`: Transforms entity data into unified book content structure
...
**Template System Integration**: ContentUtils is used by UI templates (UnifiedContent, CoverWithContent, CoverPage) to process entity data into displayable HTML content with proper portrait integration and layout optimization.
...
-   Updated version information and compatibility for WoW 11.2.0
-   Enhanced ContentUtils system for improved book template processing
-   Located ContentUtils in UI/Book/ for better organization
