...
private.Core.Logger.trace("ModuleName", "Loading timeline data for " .. expansionName)
private.Core.Logger.trace("ModuleName", "Timeline UI initialized successfully")
...
-- Available log levels: trace, warn, error
...
private.Core.StateManager.subscribe("ui.selectedEvent", function(newValue, path)
    -- Handle state change
end, "ModuleName")
...
Chronicles.Data:RegisterEventDB(collectionName, eventsDB)
Chronicles.Data:RegisterCharacterDB(collectionName, charactersDB)
Chronicles.Data:RegisterFactionDB(collectionName, factionsDB)
...
-- Status checking
local isActive = Chronicles.Data:GetEventDBStatus(collectionName)
Chronicles.Data:SetEventDBStatus(collectionName, false) -- Disable collection
```
...
local function TimedOperation()
    ...
    private.Core.Logger.trace("Performance", string.format("Operation took %.2fms", duration))
end
...
-- - events.SettingsCollectionChecked
...
private.Core.registerCallback(
    ...
    function(eventData)
        -- Handle event selection
        private.Core.Logger.trace("YourModule", "Event selected: " .. eventData.label)
    end,
