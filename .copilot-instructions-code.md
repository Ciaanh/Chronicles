...
```lua
-- Event system debugging (actually implemented)
-- Enable/disable event debugging via console
-- /ceventdebug on - Enable event debugging
-- /ceventdebug off - Disable event debugging
-- /ceventdebug history [count] - Show event history
-- /ceventdebug schemas - List available event schemas

-- State debugging via console
...
if private.Core.EventManager then
    ...
    private.Core.EventManager.Debugger:printHistory(10)
end

-- Logger configuration
private.Core.Logger.config.logLevel = "DEBUG" -- Enable debug logging
private.Core.Logger.config.enabled = true
```
...
```lua
-- Chronicles uses a sophisticated event system with validation and debugging
-- Core events are defined in private.constants.events

-- ALWAYS use centralized constants to avoid string duplication errors
local events = private.constants.events -- Local reference for cleaner code
